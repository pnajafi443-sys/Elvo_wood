const DEMO_ASSETS = window.ELVO_ASSETS || {};
document.querySelectorAll('[data-asset]').forEach(el=>{const k=el.dataset.asset;if(DEMO_ASSETS[k])el.src=DEMO_ASSETS[k];});
...
const demo=[
 ...
 {id:'demo-8',name:'سینی گرد چوبی',price:1080000,dimensions:'قطر ۳۰ سانتی‌متر',main_image:'__EMBED__:product-09.jpg'}
];
function esc(s=''){return String(s).replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]));}
function money(n){return new Intl.NumberFormat('fa-IR').format(Number(n||0))+' تومان'}
function imageUrl(path){if(!path)return '';if(path.startsWith('__EMBED__:'))return DEMO_ASSETS[path.slice(10)]||'';if(path.startsWith('http'))return path;return `${ELVO_CONFIG.supabaseUrl}/storage/v1/object/public/product-images/${path}`}
function card(p){const href=`product.html?id=${encodeURIComponent(p.id)}`;return `<article class="product-card"><a href="${href}"><div class="product-image"><img src="${imageUrl(p.main_image)}" alt="${esc(p.name)}"><span class="favorite">♡</span></div><div class="product-meta"><h3>${esc(p.name)}</h3><p>ابعاد: ${esc(p.dimensions||'—')}</p><strong>${money(p.price)}</strong><span class="view-product">مشاهده محصول</span></div></a></article>`}
...
document.querySelectorAll('[data-whatsapp]').forEach(a=>{a.href=ELVO_CONFIG.whatsapp;a.target='_blank';a.rel='noopener'});document.querySelectorAll('[data-telegram]').forEach(a=>{a.href=ELVO_CONFIG.telegram;a.target='_blank';a.rel='noopener'});
